...
import 'package:ic_fantasy_football/controller/player_update_lab.dart';
import 'dart:async';
import 'dart:convert';
...
class InternetAsync {
  ...
  Future<User> fetchUser(context, username, password) async {
    ...
    try {
      response = await http.get(
          "https://union.ic.ac.uk/acc/football/android_connect/check_user.php?username=" +
              username +
              "&password=" +
              password);
      if (response.statusCode == 200) {
  ...
  fetchPlayers(context) async {
    ...
    try {
      response = await http.get(
          "https://union.ic.ac.uk/acc/football/android_connect/players.php");
      if (response.statusCode == 200) {
        // If the call to the server was successful, parse the JSON to get user data
  ...
  fetchTeams(context) async {
    ...
    try {
      response = await http
          .get("https://union.ic.ac.uk/acc/football/android_connect/teams.php");
      if (response.statusCode == 200) {
  ...
  Future updateTeam(context) async {
    ...
    http.Response response;
    response = await http.get(
        "https://union.ic.ac.uk/acc/football/android_connect/edit_team.php?team_id=" +
            team.teamId.toString() +
            "&def_num=" +
            team.defNum.toString() +
            "&mid_num=" +
            team.midNum.toString() +
            "&fwd_num=" +
            team.fwdNum.toString() +
            "&goal=" +
            team.playerAt(0) +
            "&player1=" +
            team.playerAt(1) +
  ...
  Future addTeam(context, Team team) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        int teamId =
            int.parse(json.decode(response.body)[0]['@last_id_in_teams']);
        team.teamId = teamId;
  ...
  addUser(context, String username, String email, String password) async {
    ...
    try {
      response = await http.get(
          "https://union.ic.ac.uk/acc/football/android_connect/check_username&email.php?username=" +
              username +
              "&email=" +
              email);
      if (response.statusCode == 200) {
  ...
  addUserToDB(context, String username, String email, String password) async {
    ...
    try {
      response = await http.get(
          "https://union.ic.ac.uk/acc/football/android_connect/add_user.php?username=" +
              username +
              "&email=" +
              email +
              "&password=" +
  ...
  checkPasswordResetCode(context, String email, String resetCode) async {
    ...
    try {
      response = await http.get(
          "https://union.ic.ac.uk/acc/football/android_connect/check_passcode.php?email=" +
              email +
              "&passcode=" +
              resetCode);
      if (response.statusCode == 200) {
  ...
  resetPassword(context, String email, String password) async {
    ...
    try {
      response = await http.get(
          "https://union.ic.ac.uk/acc/football/android_connect/change_password.php?email=" +
              email +
              "&password=" +
              password);
      if (response.statusCode == 200) {
